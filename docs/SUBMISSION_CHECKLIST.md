...
- [ ] Publish up to three substantial AWS Builder Center posts with “Agents for Humans” in each title; three distinct drafts are saved in Builder Center and mirrored in `docs/builder/POST_SERIES.md`.
...
- Do not claim Bedrock model quality until a real Bedrock model run is preserved.
- AgentCore Runtime deployment and one successful cloud invocation may be claimed with the committed sanitized evidence; do not describe the private IAM endpoint as a public live demo URL.
- Do not claim physical meals delivered or real beneficiaries served.
- Do not describe AI-simulated reviewers as users or interviews.
